...
def search_privacy_law(query: str) -> str:
    ...
    knowledge = [
        (
            ["data", "privacy", "gdpr", "breach", "consent", "personal"],
            "GDPR: controllers must have a lawful basis for processing personal data, "
            "notify supervisory authorities of reportable breaches within 72 hours, and "
            "may face fines up to EUR 20M or 4% of global annual turnover. Data subjects "
            "may seek compensation for material or non-material damage.",
        ),
        (
            ["ccpa", "cpra", "california", "consumer"],
            "CCPA/CPRA: consumers have rights to know, delete, correct, and opt out of "
            "sale/sharing of personal information. Statutory damages for certain breaches "
            "can range from $100 to $750 per consumer per incident.",
        ),
        (
            ["security", "incident", "leak", "rò", "dữ liệu"],
            "Common breach response duties include containment, forensic investigation, "
            "notification to affected individuals/regulators, evidence preservation, vendor "
            "review, and remediation of security controls.",
        ),
    ]
    query_lower = query.lower()
    results = []
...
async def check_routing(state: LegalState) -> dict:
    ...
    messages = [
        SystemMessage(
            content=(
                ...
                'needs_privacy = true → question involves data, privacy, GDPR, CCPA, or data breach'
            )
        ),
        HumanMessage(content=state["question"]),
    ]
    ...
    privacy_keywords = ["data", "privacy", "gdpr", "ccpa", "breach", "dữ liệu", "rò rỉ"]
    needs_privacy = bool(parsed.get("needs_privacy", False)) or any(
        kw in state["question"].lower() for kw in privacy_keywords
    )
    print(
        "  [Node: check_routing] "
...
async def call_privacy_specialist(state: LegalState) -> dict:
    ...
    privacy_prompt = (
        ...
        "data breach response, consent, privacy notices, data subject rights, and regulator "
        "notification duties. Use the search_privacy_law tool to ground your analysis. "
...
QUESTION = (
    "If a company suffers a customer data breach and also avoids taxes on overseas revenue, "
